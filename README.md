...
|`autoset`|If this parameter has a value, then the field's value will be set to this right away (make sure the value of this parameter is also the value of an existing choice in the choice list). This is so if the field is left before time runs out or a choice is selected, its metadata will be saved immediately, as opposed to after the form instance is saved, exited, and re-opened. This is helpful if you are using the field simply as a timer in a field list. Keep in mind that metadata will not be re-saved until the form instance is saved-and-closed, so if a field is dependent on the field's metadata, set up a way so the enumerator cannot swipe forward until the other fields on the page are answered correctly.|(blank)|
